...
$format = ($_GET['format'] ?? '') === 'chevalets' ? 'chevalets' : 'affiche';
...
$lienFormat = fn(string $f) => 'affiche-qr.php?' . http_build_query(array_filter([
    'format' => $f === 'chevalets' ? 'chevalets' : null,
    'adresse' => $adresse !== $racine ? $adresse : null,
]), '', '&amp;');
?>
<!DOCTYPE html>
<html lang="fr" class="q-<?= $format ?>">
<head>
<meta charset="utf-8">
...
<div class="c-outils">
  ...
  <form class="q-form" method="get" action="affiche-qr.php">
    <?php if ($format === 'chevalets'): ?><input type="hidden" name="format" value="chevalets"><?php endif; ?>
